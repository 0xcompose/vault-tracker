<script lang="ts">
  ...
  import { clearCache, emptyCache, loadCache, mergeChain, saveCache } from "$lib/cache";
  ...
  let chainStatus = $state<Record<number, "idle" | "loading" | "done" | "error">>({});
  ...
  function syncGap(state: { lastBlock: number; headBlock: number } | undefined): number {
    if (!state) return 0;
...
<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
  <header class="mb-8">
    <h1 class="text-3xl font-semibold tracking-tight">Vault Tracker</h1>
  ...
  <section class="mb-6 flex flex-wrap items-center gap-3">
    ...
    >
      {isRefreshing ? "Refreshing…" : totalCount === 0 ? "Fetch deployments" : "Refresh"}
    </button>
    ...
    <span class="text-xs text-zinc-500">
      Last full refresh: {fmtRelative(lastFullRefresh)} · {totalCount} deployments cached
    </span>
  </section>
  ...
  <section class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
    {#each CHAINS as chain (chain.id)}
      ...
      <button
        type="button"
        class="rounded-lg border p-3 text-left transition hover:border-zinc-500 {filterChain === chain.id
          ? 'border-zinc-300 bg-zinc-900'
          : 'border-zinc-800 bg-zinc-900/40'}"
        onclick={() => (filterChain = filterChain === chain.id ? "all" : chain.id)}
      >
        ...
        <div class="mt-1 text-xl font-semibold tabular-nums">{state?.count ?? 0}</div>
        <div class="text-xs text-zinc-500">{chain.name}</div>

        <dl class="mt-2 space-y-0.5 border-t border-zinc-800 pt-2 text-[10px]">
          <div class="flex items-center justify-between">
            <dt class="text-zinc-500">last indexed</dt>
            <dd class="font-mono tabular-nums text-zinc-300">{fmtBlock(state?.lastBlock)}</dd>
          </div>
          <div class="flex items-center justify-between">
            <dt class="text-zinc-500">chain head</dt>
            <dd class="font-mono tabular-nums text-zinc-400">{fmtBlock(state?.headBlock)}</dd>
          </div>
          {#if state && gap > 0}
            <div class="flex items-center justify-between">
              <dt class="text-amber-400/80">behind</dt>
              <dd class="font-mono tabular-nums text-amber-300">{gap.toLocaleString()}</dd>
            </div>
          {:else if state && state.lastBlock > 0}
            <div class="flex items-center justify-between">
              <dt class="text-emerald-400/80">synced</dt>
              <dd class="text-emerald-400/80">{fmtRelative(state.fetchedAt)}</dd>
            </div>
          {/if}
        </dl>
  ...
  <section class="mb-3 flex items-center justify-between text-sm">
    <div class="text-zinc-400">
      Showing <span class="font-medium text-zinc-100">{deployments.length}</span>
      {#if filterChain !== "all"}
        on <span class="text-zinc-100">{CHAIN_BY_ID[filterChain]?.name}</span>
        <button
  ...
  <p class="mb-3 text-[11px] leading-relaxed text-zinc-500">
    Data source: <span class="text-zinc-300">Envio HyperSync</span> — free, keyless, server-side
    <code class="rounded bg-zinc-800 px-1 py-0.5 font-mono text-[10px]">from</code> filter. A card showing
    <span class="text-emerald-400/80">synced</span> with 0 deployments means full history (genesis → head)
    was scanned and the deployer has no contract-creation transactions on that chain. If
    <span class="text-amber-300">behind</span> is non-zero, hit Refresh to catch up.
  </p>

  <div class="overflow-x-auto rounded-lg border border-zinc-800">
    <table class="min-w-full divide-y divide-zinc-800 text-sm">
      <thead class="bg-zinc-900/60 text-left text-xs uppercase tracking-wide text-zinc-400">
        <tr>
      ...
      <tbody class="divide-y divide-zinc-900">
        {#each deployments as d (`${d.chainId}-${d.txHash}`)}
          ...
          <tr class="hover:bg-zinc-900/40">
            ...
            <td class="px-3 py-2 font-mono text-xs text-zinc-400">{fmtTime(d.timestamp)}</td>
            <td class="px-3 py-2 font-mono text-xs text-zinc-400 tabular-nums">{d.blockNumber}</td>
            <td class="px-3 py-2">
              <a
            ...
            <td class="px-3 py-2 text-right font-mono text-xs text-zinc-500 tabular-nums">
        ...
        {#if deployments.length === 0}
          <tr>
            <td colspan="6" class="px-3 py-12 text-center text-sm text-zinc-500">
              {#if totalCount === 0}
                {isRefreshing ? "Loading…" : "No deployments fetched yet. Hit Fetch."}
              {:else}
  ...
  <footer class="mt-8 text-center text-xs text-zinc-600">
    Data from Etherscan V2 multichain API. Cached in localStorage. Refresh fetches only blocks newer than cached.
  </footer>
</main>
